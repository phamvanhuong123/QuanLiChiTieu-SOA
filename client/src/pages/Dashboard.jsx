...
import { Wallet, TrendingUp, TrendingDown, ArrowRight, Settings, List, LogOut, RefreshCw, User } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [reportRes, transRes, yearlyRes] = await Promise.all([
        reportApi.getDashboard(user.id),
        transactionApi.getRecent(),     
        reportApi.getYearlyStats(user.id, currentYear) 
      ]);

      setReport(reportRes.data || { totalIncome: 0, totalExpense: 0, balance: 0 });
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500">Chào {user?.fullName}, ví của bạn hôm nay thế nào?</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <Card className="border-l-4 border-l-green-500 shadow-sm">
             <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
               <CardTitle className="text-sm font-medium text-gray-500">Tổng Thu Nhập (Tháng)</CardTitle>
           ...
           <Card className="border-l-4 border-l-red-500 shadow-sm">
             <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
               <CardTitle className="text-sm font-medium text-gray-500">Tổng Chi Tiêu (Tháng)</CardTitle>
           ...
           <Card className="border-l-4 border-l-blue-500 shadow-sm bg-blue-50/50">
             ...
             <CardContent>
               <div className="text-2xl font-bold text-blue-700">{Number(report.balance).toLocaleString()} đ</div>
             </CardContent>
           </Card>
        </div>
                    ...
                    <div key={t.id} className="flex items-center justify-between p-3 border-b last:border-0 hover:bg-gray-50 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="font-medium text-gray-800">{t.note || t.Category?.name}</span>
